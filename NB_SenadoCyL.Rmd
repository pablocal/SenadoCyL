...
pop_total_vall <- as.numeric(pop_totals[8,c(4:12, 14:18, 21, 20, 22)])
pop_total_vall <- c(sum(pop_totals[8,3:12]), pop_total_vall)
names <- c("(Intercept)", colnames(pop_totals[8,c(4:12, 14:18, 21, 20, 22)]))
names(pop_total_vall) <- names

wt_vall <- compute_mult_wt(auxvars = c("sexo_edad", "recuerdo", "tamuni"), svydata = cis, vector_calib_total = pop_total_vall, svywt = "peso_estu")


pop_total_vall <- as.numeric(pop_totals[8,c(4:12, 21, 20, 22)])
pop_total_vall <- c(sum(pop_totals[8,3:12]), pop_total_vall)
names <- c("(Intercept)", colnames(pop_totals[8,c(4:12, 21, 20, 22)]))
names(pop_total_vall) <- names

wt_vall_socdem <- compute_mult_wt(auxvars = c("sexo_edad", "tamuni"), svydata = cis, vector_calib_total = pop_total_vall, svywt = "peso_estu")

peso_vall_socdem <- weights(wt_vall_socdem$svy_des_wt)
peso_vall <- weights(wt_vall$svy_des_wt)

cis <- cbind(cis, peso_vall_socdem)
cis <- cbind(cis, peso_vall)

cis %>% frq(sexo_edad, weights = peso_burgos_socdem)
...
indep <- cis %>%  select(-estu, -voto, -id, -peso_estu, -sexo_edad, -peso_burgos, -peso_burgos2, -part, -starts_with("peso"))
indep <- netCoin::dichotomize(indep, colnames(indep))
indep <- as.matrix(select(indep, 44:187))
dep <-  as.matrix(cis %>% select(part))
wts <- as.numeric(cis$peso_burgos_socdem)
...
## ntiles prob
cis <- cis %>%  mutate(part_ntiles = ntile(part_prob, 100))
part_ntiles <- grattan::weighted_ntile(cis$part_prob, cis$peso_burgos_socdem, 100)
cis <- cbind(cis, part_ntiles)
cis %>% group_by(part_ntiles) %>% summarise(part_prob_mean = mean(part_prob))

cis %>% filter(part_ntiles > 30 & voto != "No votaría") %>% frq(voto, weights = peso_vall_socdem, title = "VALLADOLID (70% part.)")
